...
def _run_with_mode(code, mode, monkeypatch, enabled_tools=None):
    """Execute code under a given mode (via the env override)."""
    monkeypatch.setenv("HERMES_EXECUTE_CODE_MODE", mode)
...
class TestGetExecutionMode(unittest.TestCase):
    """_get_execution_mode reads env > config > default."""

    def test_default_is_project(self):
        self.assertEqual(DEFAULT_EXECUTION_MODE, "project")

    def test_env_override_strict(self):
        with patch.dict(os.environ, {"HERMES_EXECUTE_CODE_MODE": "strict"}):
            self.assertEqual(_get_execution_mode(), "strict")

    def test_env_override_project(self):
        with patch.dict(os.environ, {"HERMES_EXECUTE_CODE_MODE": "project"}):
    ...
    def test_invalid_env_falls_back(self):
        """Bogus env value → default, not a crash."""
        with patch.dict(os.environ, {"HERMES_EXECUTE_CODE_MODE": "banana"}):
            self.assertEqual(_get_execution_mode(), DEFAULT_EXECUTION_MODE)

    def test_empty_env_reads_config(self):
        """Empty env var is treated as unset, falls through to config layer."""
        with patch.dict(os.environ, {"HERMES_EXECUTE_CODE_MODE": ""}):
            with patch("tools.code_execution_tool._load_config", return_value={"mode": "strict"}):
                self.assertEqual(_get_execution_mode(), "strict")

    def test_config_strict(self):
        """code_execution.mode=strict in config.yaml."""
        with patch.dict(os.environ, clear=False) as env:
            env.pop("HERMES_EXECUTE_CODE_MODE", None)
            with patch("tools.code_execution_tool._load_config", return_value={"mode": "strict"}):
                self.assertEqual(_get_execution_mode(), "strict")

    def test_config_invalid_falls_back(self):
        """Bogus config value → default."""
        with patch.dict(os.environ, clear=False) as env:
            env.pop("HERMES_EXECUTE_CODE_MODE", None)
            with patch("tools.code_execution_tool._load_config", return_value={"mode": "banana"}):
                self.assertEqual(_get_execution_mode(), DEFAULT_EXECUTION_MODE)
...
class TestResolveChildPython(unittest.TestCase):
    ...
    def test_project_with_virtualenv_picks_venv_python(self, tmp_path=None):
...
class TestModeAwareSchema(unittest.TestCase):
    ...
    def test_default_mode_reads_config(self):
        """build_execute_code_schema() with mode=None reads config."""
        with patch.dict(os.environ, {"HERMES_EXECUTE_CODE_MODE": "strict"}):
...
class TestExecuteCodeModeIntegration(unittest.TestCase):
    ...
    def _run(self, code, mode, enabled_tools=None, extra_env=None):
        env_overrides = {"HERMES_EXECUTE_CODE_MODE": mode}
        if extra_env:
...
class TestSecurityInvariantsAcrossModes(unittest.TestCase):
    ...
    def _run(self, code, mode):
        with patch.dict(os.environ, {"HERMES_EXECUTE_CODE_MODE": mode}):
            with patch("model_tools.handle_function_call",
                       side_effect=_mock_handle_function_call):
                raw = execute_code(
                    code=code,
