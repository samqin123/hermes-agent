...
def _get_execution_mode() -> str:
    ...
    Resolution order:
      1. ``HERMES_EXECUTE_CODE_MODE`` env var (test/operator override)
      2. ``code_execution.mode`` in config.yaml
      3. ``DEFAULT_EXECUTION_MODE`` ('project')

    Invalid values fall back to ``DEFAULT_EXECUTION_MODE`` with a log warning.
    ...
    Env scrubbing and tool whitelist apply identically in both modes.
    """
    raw = os.environ.get("HERMES_EXECUTE_CODE_MODE", "").strip().lower()
    if raw:
        if raw in EXECUTION_MODES:
            return raw
        logger.warning(
            "Ignoring HERMES_EXECUTE_CODE_MODE=%r (expected one of %s)",
            raw, EXECUTION_MODES,
        )
